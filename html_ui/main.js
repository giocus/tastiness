...
(function() {
  ...
  ramBlackoutBytesChanged = function() {
    RamBlackoutBytes.sort(function(a, b) {
      return a - b;
    });
    RamBlackoutBytes = _.uniq(RamBlackoutBytes);
    $RamSpans.removeClass('blackout');
    _.each(RamBlackoutBytes, function(i) {
  ...
  setFrameInput = function(frame, input) {
    var change, newv;
    change = false;
    ...
    if (typeof e.data === 'string') {
      if (e.data.match(/^bin/)) {
        ...
        try {
          ...
          return console.log("unknown string: " + e.data);
        }
      }
    } else {
      if (false) {
  ...
  frameChanged = function(v) {
    ...
    return _.each(FrameScrollerNumbers, function(frameNumDiv, i) {
      var n;
      n = i + CurFrame - FrameScrollerFocus;
      if (n < 0) {
